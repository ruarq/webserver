...
char const *http_status_to_string(http_status_t const status)
{
...
int http_response_from_string(http_response_t *response, char const *str)
{
	...
	http_parser_result_t const result = http_parser_parse_response(&parser, response);
...
char *http_response_to_string(http_response_t const *response)
{
	...
	size_t const required_length = strlen(version_string) + 1 + strlen(status_num_string) + 1 +
...
int http_responses_from_string(http_response_t *responses, size_t *n, char const *str)
{
	...
	parser.source	  = str;
	parser.source_len = strlen(str);
	return http_parser_parse_responses(&parser, responses, n);
}

char *http_responses_to_string(http_response_t const *responses, size_t const n)
{
	char  *buf	       = NULL;
	size_t required_length = 0;

	for (size_t i = 0; i < n; ++i) {
		char *single_response_buf = http_response_to_string(&responses[i]);
